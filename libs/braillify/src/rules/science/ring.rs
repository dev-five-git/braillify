...
//! U+2B21 `⬡`, 가로 방향은 U+2394 `⎔`, 오각 환핵은 U+2B20 `⬠`, 사각 환핵은 U+25A1 `□`,
//! 삼각 환핵은 U+25B3 `△` 로 적는다. 같은 줄에서 세로 결합선을 나눈 이웃은 두 칸, 사선
//! 결합선을 나눈 이웃은 한 칸·한 줄 떨어진다.
...
fn ring(glyph: char) -> Option<Ring> {
    match glyph {
        ...
        '\u{25A1}' => Some(Ring::Square),
        '\u{25B3}' => Some(Ring::Triangle),
        _ => None,
    }
}
...
mod tests {
    ...
    #[case::squares_do_not_fuse(" \u{25A1}\n\u{25A1}")]
