/** Reports 점사랑 exact-match accuracy against the NIKL corpus reference. */
...
const REPORT_PATH = join(ROOT, 'bench', 'JEOMSARANG_CORPUS_BENCH.md')
const BRAILLE_BLANK = '\u2800'
...
/**
 * 점사랑 라이브러리는 한글 모드 점역만 제공하므로, 영문으로 시작/끝나는 입력의
 * 외곽 로마자표/종료표는 양쪽에서 제거한 뒤 비교한다 (jeomjasesang-corpus-bench.ts 와 동일).
 */
...
async function measureYear(directory: string, year: string): Promise<YearStats> {
  ...
  for (const file of corpusFiles) {
    ...
    for (const { input, unicode, jeomsarang } of corpus) {
      stats.total++
      if (jeomsarang == null || jeomsarang === '') {
        stats.missing++
      } else if (
        stripOuterEnglishMarkers(normalizeBraille(jeomsarang), input) ===
...
async function main(): Promise<void> {
  ...
  for (const stats of perYear) {
    console.log(
      `점사랑 ${stats.year}: ${stats.match}/${stats.match + stats.mismatch} (${accuracyOf(stats).toFixed(2)}%), ${stats.missing} missing`,
    )
  }
  console.log(
    `점사랑 합계: ${grand.match}/${grand.match + grand.mismatch} (${accuracyOf(grand).toFixed(2)}%), ${grand.missing} missing`,
  )
}
